 ...
 *
 * Provides additional actions related to moderation for @Euan T's MyAlerts plugin.
 *
...
function modpack_change_username(&$args)
{
  ...
  // member.php?action=resetpassword does not load MyAlerts. This ensures we always have it in place
	$instance = MybbStuff_MyAlerts_AlertTypeManager::getInstance();

  if (!$instance) {
    $instance = MybbStuff_MyAlerts_AlertTypeManager::createInstance($db, $cache);
  }

  $instance->getByCode('changeusername');
